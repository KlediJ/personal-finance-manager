...
import { LoanRepository, AmortizationScheduleRepository } from '../../src/data-storage/repositories/LoanRepository';
import { LoanDetails, AmortizationSchedule, LoanType } from '../../src/data-storage/models/LoanDetails';

export function setupLoanHandlers(): void {
  const loanRepository = new LoanRepository();
  const amortizationRepository = new AmortizationScheduleRepository();
  ...
  ipcMain.handle('loans:getById', async (_, id: number) => {
    try {
  ...
  ipcMain.handle('loans:create', async (_, loan: LoanDetails) => {
    try {
  ...
  ipcMain.handle('loans:update', async (_, id: number, loan: LoanDetails) => {
    try {
  ...
  ipcMain.handle('loans:getByAccount', async (_, accountId: number) => {
    try {
  ...
  ipcMain.handle('loans:getByType', async (_, loanType: LoanType) => {
    try {
  ...
  ipcMain.handle('loans:getActiveLoan', async (_, accountId: number) => {
    try {
  ...
  ipcMain.handle('loans:getWithSummary', async () => {
    try {
  ...
  ipcMain.handle('loans:updateBalance', async (_, loanId: number, newBalance: number) => {
    try {
  ...
  ipcMain.handle('loans:generateAmortizationSchedule', async (_, loanId: number) => {
    try {
  ...
  ipcMain.handle('amortization:getAll', async () => {
    try {
  ...
  ipcMain.handle('amortization:getById', async (_, id: number) => {
    try {
  ...
  ipcMain.handle('amortization:create', async (_, schedule: AmortizationSchedule) => {
    try {
  ...
  ipcMain.handle('amortization:update', async (_, id: number, schedule: AmortizationSchedule) => {
    try {
  ...
  ipcMain.handle('amortization:delete', async (_, id: number) => {
    try {
  ...
  ipcMain.handle('amortization:getByLoan', async (_, loanId: number) => {
    try {
  ...
  ipcMain.handle('amortization:getActualPayments', async (_, loanId: number) => {
    try {
  ...
  ipcMain.handle('amortization:markPaymentActual', async (_, scheduleId: number, transactionId: number) => {
    try {
  ...
  ipcMain.handle('amortization:bulkInsert', async (_, scheduleItems: Omit<AmortizationSchedule, 'schedule_id'>[]) => {
    try {
