...
exports.MistralProcessor = void 0;
class MistralProcessor {
    ...
    suggestCategoryForMerchant(merchantName, description) {
    ...
    extractPayeeInfo(description, existingPayees) {
    ...
    matchExistingPayee(description, existingPayees) {
        let bestMatch = null;
        let bestScore = 0;
    ...
    calculatePayeeMatchScore(description, payeeName) {
    ...
    predictCategory(description, availableCategories, payeeExtraction) {
        const predictions = [];
    ...
    getKeywordBasedPredictions(description, availableCategories) {
        const predictions = [];
        ...
        for (const [categoryType, data] of scores) {
            const matchingCategory = availableCategories.find(c => c.name.toLowerCase().includes(categoryType) ||
    ...
    getPatternBasedPredictions(description, availableCategories) {
        ...
        if (amountMatch) {
            ...
            if (amount >= 1000) {
                const housingCategory = availableCategories.find(c => c.name.toLowerCase().includes('housing') ||
        ...
        if (description.includes('monthly') || description.includes('recurring')) {
            const billsCategory = availableCategories.find(c => c.name.toLowerCase().includes('bills') ||
    ...
    extractAdditionalInfo(description) {
        const info = {};
    ...
    calculateOverallConfidence(categoryPredictions, payeeExtraction, extractedInfo) {
    ...
    async batchProcessTransactions(transactions, availableCategories, existingPayees, onProgress) {
