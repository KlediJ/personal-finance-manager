...
import { LoanDetails } from '../../data-storage/models/LoanDetails';
...
export interface CodeLlamaResult {
  type: 'analysis' | 'recommendation' | 'calculation' | 'insight' | 'forecast';
  content: string;
  data?: any;
  confidence: number;
  actionItems?: string[];
...
export class CodeLlamaProcessor {
  ...
  public async processComplexQuery(query: string): Promise<CodeLlamaResult> {
  ...
  private async analyzeDebtSituation(query: string): Promise<CodeLlamaResult> {
    // Analyze all debt-related accounts and transactions
    const debtAccounts = this.context.accounts.filter(acc => 
    ...
    const highestDebt = debtAccounts.reduce((max, acc) => 
      Math.abs(acc.current_balance) > Math.abs(max.current_balance) ? acc : max
    );
  ...
  private async optimizeLoanPayments(query: string): Promise<CodeLlamaResult> {
    const loanAccounts = this.context.accounts.filter(acc => 
      acc.type.toLowerCase().includes('loan')
    );
  ...
  private async forecastBillPayments(query: string): Promise<CodeLlamaResult> {
  ...
  private async analyzeSpendingPatterns(query: string): Promise<CodeLlamaResult> {
  ...
  private async analyzeFinancialRelationships(query: string): Promise<CodeLlamaResult> {
    const relationships = {
  ...
  private async analyzeInterestExpenses(query: string): Promise<CodeLlamaResult> {
    const interestTransactions = this.context.transactions.filter(t => 
  ...
  private async analyzeCreditUtilization(query: string): Promise<CodeLlamaResult> {
    const creditAccounts = this.context.accounts.filter(acc => 
      acc.type.toLowerCase().includes('credit')
    );
  ...
  private async assessFinancialHealth(query: string): Promise<CodeLlamaResult> {
    const healthMetrics = {
  ...
  private async handleGeneralQuery(query: string): Promise<CodeLlamaResult> {
    return {
      type: 'insight',
  ...
  private estimateInterestRate(account: Account): number {
    // Simplified interest rate estimation based on account type
    if (account.type.toLowerCase().includes('credit')) return 0.18;
    if (account.type.toLowerCase().includes('loan')) return 0.06;
    return 0.05;
  }

  private estimateMonthlyPayment(account: Account): number {
  ...
  private forecastUpcomingBills(recurringTransactions: any[]): any[] {
  ...
  private analyzeBillPatterns(recurringTransactions: any[]): any {
    return {
  ...
  private calculateCategorySpending(expenses: Transaction[]): any[] {
  ...
  private calculateMonthlyTrends(expenses: Transaction[]): any[] {
  ...
  private identifySeasonalPatterns(expenses: Transaction[]): any {
  ...
  private getMonthOccurrences(month: number, expenses: Transaction[]): number {
    const years = new Set();
    expenses.forEach(expense => {
      const date = new Date(expense.date);
  ...
  private identifySpendingAnomalies(expenses: Transaction[]): any[] {
  ...
  private calculateAverageMonthlySpending(expenses: Transaction[]): number {
  ...
  private mapAccountPayeeRelationships(): any[] {
  ...
  private mapCategoryPayeeRelationships(): any[] {
    const relationships = new Map<string, Set<string>>();
  ...
  private mapBillAccountRelationships(): any[] {
    // This would use the bills data when available
    return [];
  }

  private analyzeTransactionNetwork(): any {
    return {
  ...
  private calculateMonthlyInterest(interestTransactions: Transaction[]): number {
  ...
  private groupInterestByAccount(interestTransactions: Transaction[]): any[] {
  ...
  private findHighestInterestAccount(interestByAccount: any[]): string {
    if (interestByAccount.length === 0) return 'None';
    return interestByAccount.reduce((max, acc) => 
      acc.totalInterest > max.totalInterest ? acc : max
    ).accountName;
  }

  private calculatePotentialInterestSavings(interestByAccount: any[]): number {
  ...
  private estimateCreditLimit(account: Account): number {
  ...
  private calculateDebtToIncomeRatio(): number {
    const monthlyIncome = this.calculateMonthlyIncome();
    const debtAccounts = this.context.accounts.filter(acc => 
      acc.type.toLowerCase().includes('credit') || acc.type.toLowerCase().includes('loan')
    );
    const totalDebt = debtAccounts.reduce((sum, acc) => sum + Math.abs(acc.current_balance), 0);
    return monthlyIncome > 0 ? (totalDebt / (monthlyIncome * 12)) * 100 : 0;
  }

  private calculateEmergencyFundCoverage(): number {
    const monthlyExpenses = this.calculateAverageMonthlySpending(
      this.context.transactions.filter(t => t.amount < 0)
    );
    const savingsAccounts = this.context.accounts.filter(acc => 
      acc.type.toLowerCase().includes('saving') || acc.type.toLowerCase().includes('checking')
    );
  ...
  private calculateOverallCreditUtilization(): number {
    const creditAccounts = this.context.accounts.filter(acc => 
      acc.type.toLowerCase().includes('credit')
    );
  ...
  private calculateFinancialHealthScore(metrics: any): number {
    let score = 0;
    let components = 0;

    // Debt-to-income ratio (0-25 points)
    if (metrics.debtToIncomeRatio <= 20) score += 25;
    else if (metrics.debtToIncomeRatio <= 36) score += 15;
    else if (metrics.debtToIncomeRatio <= 50) score += 10;
    components++;

    // Emergency fund (0-25 points)
    if (metrics.emergencyFundCoverage >= 6) score += 25;
    else if (metrics.emergencyFundCoverage >= 3) score += 15;
    else if (metrics.emergencyFundCoverage >= 1) score += 10;
    components++;
  ...
  public updateContext(newContext: FinancialContext): void {
