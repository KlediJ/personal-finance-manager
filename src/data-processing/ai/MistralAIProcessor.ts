...
export interface MistralFinancialResult {
...
export class MistralAIProcessor {
   ...
   */
  public async chat(messages: MistralChatMessage[]): Promise<MistralResponse> {
    try {
  ...
  ): Promise<MistralFinancialResult> {
    try {
  ...
  private formatMessagesAsPrompt(messages: MistralChatMessage[]): string {
    let prompt = "";
  ...
  private createCategorizationPrompt(
    transaction: Transaction,
    availableCategories: Category[],
    existingPayees: Payee[]
  ): string {
...
[ASSISTANT] `;
  ...
  private async runMistralInference(model: any, prompt: string): Promise<{ text: string; usage?: any }> {
    try {
  ...
  private simulateIntelligentResponse(prompt: string): { text: string; usage?: any } {
  ...
  private simulateChatResponse(prompt: string): { text: string; usage?: any } {
  ...
  private simulateCategorizationResponse(prompt: string): { text: string; usage?: any } {
  ...
  private parseCategorizationResponse(
    ...
    try {
      ...
      const categoryPrediction: CategoryPrediction = {
        category: category as Category,
        confidence: parsed.category_confidence / 100,
        reasoning: parsed.reasoning || 'AI categorization'
      };
      
      let payeeExtraction: PayeeExtraction | null = null;
      if (parsed.payee_name) {
        // Check if payee already exists
        const existingPayee = existingPayees.find(p => 
          p.name.toLowerCase().includes(parsed.payee_name.toLowerCase()) ||
      ...
      return this.fallbackCategorization({ description: responseText } as Transaction, availableCategories, existingPayees);
    }
  }
  ...
  private mockChatResponse(messages: MistralChatMessage[]): MistralResponse {
