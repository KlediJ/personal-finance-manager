...
import { InterestExpense, InterestExpenseSummary, InterestExpenseType, CreditCardInterestAnalysis, PayoffScenario } from '../models/InterestExpense';

export class InterestExpenseRepository extends BaseRepository<InterestExpense> {
  constructor() {
    super('interest_expenses');
  }

  protected mapToEntity(row: any): InterestExpense {
    return {
      expense_id: row.expense_id,
      account_id: row.account_id,
      transaction_id: row.transaction_id,
      expense_type: row.expense_type as InterestExpenseType,
  ...
  public getExpensesByAccount(accountId: number): InterestExpense[] {
    return this.findBy('account_id', accountId);
  }

  public getExpensesByType(expenseType: InterestExpenseType): InterestExpense[] {
    return this.findBy('expense_type', expenseType);
  }

  public getExpensesByDateRange(startDate: string, endDate: string): InterestExpense[] {
    const query = `
  ...
  public getExpensesWithSummary(): InterestExpenseSummary[] {
    const query = `
      SELECT 
        ie.*,
  ...
  public getTotalInterestByAccount(accountId: number, year?: number): number {
    let query = `
    ...
    const statement = this.db.prepare(query);
    const result = statement.get(...params) as { total: number } | undefined;
    
    return result?.total || 0;
  }

  public getTotalInterestByType(expenseType: InterestExpenseType, year?: number): number {
    let query = `
      SELECT SUM(interest_amount) as total
      FROM interest_expenses
      WHERE expense_type = ?
  ...
  public getMonthlyInterestTrend(accountId: number, months: number = 12): Array<{month: string, amount: number}> {
    const query = `
      SELECT 
  ...
  public calculateCreditCardInterestAnalysis(accountId: number): CreditCardInterestAnalysis | null {
    // Get account details
    const accountQuery = `
      SELECT 
        a.*,
    ...
    const accountData = accountStatement.get(accountId) as any;
    ...
    const currentMonthResult = currentMonthStatement.get(accountId) as { current_month_interest: number } | undefined;
    ...
    const ytdResult = ytdStatement.get(accountId) as { ytd_interest: number } | undefined;
  ...
  private calculatePayoffScenarios(balance: number, annualRate: number, minimumPayment: number): PayoffScenario[] {
    const scenarios: PayoffScenario[] = [];
    const monthlyRate = annualRate / 100 / 12;
    
    // Scenario 1: Minimum payments only
    scenarios.push(this.calculatePayoffScenario(
  ...
  private calculatePayoffScenario(
    scenarioName: string,
    balance: number,
    monthlyRate: number,
