import { AIRepository, AIPerformanceMetric, AILearningData } from '../repositories/AIRepository';
...
export class AIMetricsService {
   ...
   */
  async getCategorizationMetrics(modelName: string, days = 30): Promise<CategorizationMetrics> {
    ...
    const recentData = learningData.filter(d => 
   ...
   */
  async generatePerformanceReport(modelName: string, days = 30): Promise<ModelPerformanceReport> {
    ...
    const recentFeedback = feedbackData.filter(d => 
      new Date(d.created_at!) >= startDate
    );
   ...
   */
  async getConfidenceTrend(modelName: string, days = 30): Promise<Array<{ date: string; confidence: number }>> {
   ...
   */
  async getAccuracyTrend(modelName: string, days = 30): Promise<Array<{ date: string; accuracy: number }>> {
    const learningData = this.aiRepo.getLearningDataForModel(modelName, 'categorization');
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
   ...
   */
  async cleanup(olderThanDays = 90): Promise<{ metricsDeleted: number; learningDataDeleted: number }> {
  ...
  private async updateCategorizationMetrics(modelName: string): Promise<void> {
  ...
  private generateRecommendations(
    categorization: CategorizationMetrics,
    cacheStats: { hitRate: number; totalEntries: number; expiredEntries: number },
    learning: { feedbackCount: number; correctionsApplied: number; accuracyImprovement: number }
  ): string[] {
